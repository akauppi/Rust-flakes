...
async fn a_main() -> Result<()> {
    ...
    //E let pool: Pool = Pool::new();   // inner mutable
    ...
    future::join(fut1, fut2) .await;
        // Rust note: using 'futures_util::future', which brings less dependencies than 'futures::future'
...
async fn fish_me_some(fish: Fish) -> Fish {
    info!("Fishing for {fish}...");

    // Note: CANNOT (?) wait within async function without great dependencies? 'async_std'
    fish
}
